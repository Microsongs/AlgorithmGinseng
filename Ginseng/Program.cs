...
namespace 산삼
{
    ...
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            for (int i = 0; i < rootNum; i++)
            {
                ...
                Point randomRoot = startPoint[random.Next(startPoint.Count)]; 
                int rootLength = random.Next(0, 4) + 6;
                Console.WriteLine("시작위치개수 : {0}", startPoint.Count);
                CreateRoot(ref ginseng, direction,ref randomRoot, rootLength, 1);
            }
        ...
        static void CreateRoot(ref char[,] map , List<Point> direction,ref Point randomRoot, int rootLength,int order)
        {
        ...
        static void StartPointPush(ref List<Point> p)
        {
            for (int j = 0; j < 4; j++)
            {
                p.Add(new Point(minLoc - 1, minLoc + j));
                p.Add(new Point(minLoc + j, minLoc - 1));
                p.Add(new Point(minLoc + j, maxLoc + 1));
                p.Add(new Point(maxLoc + 1, minLoc + j));
            }
        }
        ...
        static bool BFS(ref char[,] map, ref List<Point> startPoint, List<Point> direction)
        {
            ...
            for (int i = 0; i < startPoint.Count; i++)
            {
                ...
                while (queue.Count != 0 && queue.Count <= 6)
                {
                    ...
                    if (temp.X > 0)
                    {
                        if (map[temp.X + direction[0].X, temp.Y + direction[0].Y] == '0')
                        {
                            queue.Enqueue(temp);
                    ...
                    if (temp.X < 11)
                    {
                        if (map[temp.X + direction[1].X, temp.Y + direction[1].Y] == '0')
                        {
                            queue.Enqueue(temp);
                    ...
                    if (temp.Y < 11)
                    {
                        if (map[temp.X + direction[2].X, temp.Y + direction[2].Y] == '0')
                        {
                            queue.Enqueue(temp);
                    ...
                    if (temp.Y > 0)
                    {
                        if (map[temp.X + direction[3].X, temp.Y + direction[3].Y] == '0')
                        {
                            queue.Enqueue(temp);
